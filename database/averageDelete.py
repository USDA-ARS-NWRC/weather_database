...
def get_stations(table,str_statement):
    """
    Get stations from table according to some condition provided by str_statement
    Example:
        1. get_stations("tbl_level1","av_del=0")
            returns all the stations where average delete was not be ran.
        2. get_stations("tbl_level1","clipped=0")
            returns all the stations where Min Max clipping was not be applied
    """
    ...
    qry_station = "SELECT DISTINCT(station_id) from {0} where {1}".format(table,str_statement)
...
if __name__ == "__main__":
    ...
    # get the stations
    stations = get_stations("tbl_level1","av_del=0")
